  ...
  return (
    <div className={`flex flex-col md:flex-row h-screen overflow-hidden ${themeMode === "dark" ? "bg-[#09090d] text-white" : "bg-slate-900 text-slate-100"} relative`}>
      ...
      <div className={`resume-editor-sidebar w-full md:w-[480px] md:shrink-0 flex flex-col border-r border-slate-700 bg-slate-800 h-full ${mobileTab === "edit" ? "flex" : "hidden md:flex"}`}>
        ...
        {/* Tab Headers */}
        <div className="flex flex-wrap gap-1.5 p-3 border-b border-slate-700 bg-slate-900 shrink-0">
          {["theme", "profile", "summary", "experience", "education", "projects", "skills", "others"].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-3.5 py-1.5 rounded-lg text-[9px] sm:text-[10px] uppercase font-mono tracking-wider font-bold transition cursor-pointer ${
                activeTab === tab 
