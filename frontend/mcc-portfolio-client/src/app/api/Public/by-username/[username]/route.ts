...
export async function GET(
  ...
  try {
    ...
    // 1. Try match by username field
    let user = await prisma.users.findFirst({
      where: { Username: { equals: username, mode: "insensitive" } }
    });

    // 2. Try match by fullname no spaces/dashes
    if (!user) {
      const allUsers = await prisma.users.findMany();
      user = allUsers.find(x => 
        x.FullName.replace(/\s+/g, "").replace(/-/g, "").toLowerCase() === username
      ) || null;
    }

    // 3. Try match by register number
    if (!user) {
      user = await prisma.users.findFirst({
        where: { RegisterNumber: { equals: username, mode: "insensitive" } }
      });
    }
    ...
    const portfolio = await fetchFullPortfolio(user.Id);
    if (!portfolio) {
      return NextResponse.json("User not found", { status: 404 });
    }
